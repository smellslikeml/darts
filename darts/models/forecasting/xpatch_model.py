...
class _XPatchModule(PLForecastingModule):
    ...
    def forward(self, x_in: PLModuleInput) -> PLModuleOutput:
        """
        x_in
            a ``PLModuleInput`` of independent feature tensors. ``concatenate_past_features()``
            joins the past target, past covariates and historic future covariate chunk along
            the component dimension into ``x`` of shape ``(n_samples, n_time_steps, n_variables)``.
        """
        ...
        return PLModuleOutput(prediction=x[:, :, : self.output_dim, :])
